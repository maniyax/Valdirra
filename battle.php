...
//$moneyfor += $moneyfor * $me['pererod'] * 0.05;
			...
			if($uz['flag_bot'] == 0)
				...
				$q = $db->query("update `users` set cu=cu+'{$moneyfor}',nalog=nalog+'{$nalog}',chest=chest+'{$points}' where login='{$uz['login']}' limit 1;");
				}
			else
				{
...
//$moneyfor += $moneyfor * $uz['pererod'] * 0.05;
			if($moneyfor < 1) $moneyfor = 1;
			...
			$q = $db->query("update `users` set cu=cu+'{$moneyfor}',nalog=nalog+'{$nalog}',chest=chest+'{$points}' where login='{$me['login']}' limit 1;");
...
msg('HP: <b><span style="color:'.$hpcolor.'">'.$me['hpnow'].'</span></b>/<b><span style="color:'.$hpcolor.'">'.$me['hpmax'].'</span></b> | 
MP: <b><span style="color:'.$manacolor.'">'.$me['mananow'].'</span></b>/<b><span style="color:'.$manacolor.'">'.$me['manamax'].'</span></b><br>');
...
$f['exp'] -= $f['exp']*0.1;
		...
		$q = $db->query("update `combat`,`users` set `users`.`lost`=`users`.`lost`+1,`users`.`exp`='{$f['exp']}',`users`.`doping`=0,`users`.`doping_time`=0,`users`.`rabota`=0,`users`.`loc`={$f['priv']},`users`.`kvest_now`='',`users`.`kvest_step`=0 where (`combat`.`flag_bot`=0 and `combat`.`komanda`=2 and `combat`.`boi_id`='{$bid}' and `users`.`login`=`combat`.`login`);");
		}
...
$f['exp'] -= $f['exp']*0.1;
		...
		$q = $db->query("update `combat`,`users` set `users`.`lost`=`users`.`lost`+1,`users`.`exp`='{$f['exp']}',`users`.`doping`=0,`users`.`doping_time`=0,`users`.`rabota`=0,`users`.`loc`={$f['priv']},`users`.`kvest_now`='',`users`.`kvest_step`=0 where (`combat`.`flag_bot`=0 and `combat`.`komanda`=1 and `combat`.`boi_id`='{$bid}' and `users`.`login`=`combat`.`login`);");
		}
